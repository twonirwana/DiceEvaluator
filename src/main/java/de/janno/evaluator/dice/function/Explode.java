...
public class Explode extends Function {
    ...
    public @NonNull RollBuilder evaluate(@NonNull List<RollBuilder> arguments, @NonNull String inputValue) throws ExpressionException {
        return variables -> {
            ...
            final List<Roll> firstRoll = inputRoll.extendRoll(variables).orElse(Collections.emptyList());
            rollExpression.addAll(firstRoll);
            allChildrenRollBuilder.addAll(firstRoll);

            Optional<List<Roll>> compareToRoll = arguments.get(1).extendRoll(variables);
            if (compareToRoll.isEmpty()) {
                throw new ExpressionException(String.format("'%s' requires a non-empty input as second argument", inputValue));
            }
            checkRollSize(inputValue, compareToRoll.get(), 1, 1);
            ...
            if (arguments.size() == 3) {
                Optional<List<Roll>> maxRerollsRoll = arguments.get(2).extendRoll(variables);
                if (maxRerollsRoll.isEmpty()) {
                    throw new ExpressionException(String.format("'%s' requires a non-empty input as third argument", inputValue));
                }
                checkRollSize(inputValue, maxRerollsRoll.get(), 1, 1);
                Roll maxNumberOfRerollsRoll = maxRerollsRoll.get().getFirst();
                maxNumberOfRerolls = maxNumberOfRerollsRoll.asInteger().orElseThrow(() -> throwNotIntegerExpression(inputValue, maxRerollsRoll.get().getFirst(), "third argument"));
                if (maxNumberOfRerolls > 100 || maxNumberOfRerolls < 0) {
                    throw new ExpressionException(String.format("'%s' requires as third argument a number between 0 and 100", inputValue));
                }
            ...
            while (rerolls < maxNumberOfRerolls && lastRoll.stream().flatMap(r -> r.getElements().stream()).anyMatch(compareTo::isElementsContainsElementWithValueAndTag)) {
                lastRoll = inputRoll.extendRoll(variables).orElse(Collections.emptyList());
            ...
            return Optional.of(ImmutableList.of(new Roll(getExpression(inputValue, rollExpression.build()),
                    allResultRolls.stream().flatMap(r -> r.getElements().stream()).collect(ImmutableList.toImmutableList()),
                    UniqueRandomElements.from(allChildrenRollBuilder.build()),
                    allChildrenRollBuilder.build())));
        };
    }
}
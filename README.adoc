...
|if equal |`ifE(<expression>,<expression>,<true>,<false>)` |`ifE(1d6,6,'six','no six')` or `ifE(1d6,6,'six')` |compares the elements of two `<expression>` and if they are equal return the `<true>` expression or else the `<false>` expression. The <false> expression is optional, if it is missing and the elements are not equal, the result will be the input expression.
|if greater |`ifG(<expression>,<expression>,<true>,<false>)` |`ifG(1d6,4,'high','low')` or `ifG(1d6,4,'high')`|compares the elements of two `<expression>` and if the left is greater than the right then return the `<true>` expression or else the `<false>` expression. The <false> expression is optional, if it is missing and the elements are not equal, the result will be the input expression. Both `<expression>` need to have a single element as result.
|if lesser |`ifL(<expression>,<expression>,<true>,<false>)` |`ifL(1d6,3,'low','high')` or `ifL(1d6,3,'low')`|compares the elements of two `<expression>` and if the left is lesser than the right then return the `<true>` expression or else the `<false>` expression. The <false> expression is optional, if it is missing and the elements are not equal, the result will be the input expression. Both `<expression>` need to have a single element as result.
...
** => possible solution: defining constance in the expression: \{3d6:diceA} (latexmath:[$diceA$]=)+(ifEqual(latexmath:[$diceA$]=6c,3,`critical success',`no critical success'),’’)
